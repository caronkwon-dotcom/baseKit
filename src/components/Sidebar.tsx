...
export default function Sidebar({
  ...
  return (
    <aside className="sidebar">
      <div className="sidebar-heading"><span>{menu.menuName}</span><div className="navigation-heading-actions"><button type="button" className={pinned ? 'active' : ''} title={pinned ? '사이드바 고정 해제' : '사이드바 고정'} aria-label={pinned ? '사이드바 고정 해제' : '사이드바 고정'} aria-pressed={pinned} onClick={pinned ? onUnpin : onPin}>⌖</button></div></div>
