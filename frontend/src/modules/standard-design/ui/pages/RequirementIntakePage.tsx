...
import { requirementApi, type Requirement, type RequirementAttachment, type RequirementInput } from '../../requirement/requirementApi';
...
function legacyRequirements(): LegacyRequirement[] {
  try { return (JSON.parse(localStorage.getItem(legacyKey) ?? '{}') as { requirements?: LegacyRequirement[] }).requirements ?? []; }
  catch { return []; }
}
...
export default function RequirementIntakePage() {
  ...
  const [selectedFile, setSelectedFile] = useState<RequirementAttachment | null>(null);
  ...
  const load = async (id = '') => {
    ...
    const next = await requirementApi.list(projectId);
    setRows(next);
    if (id) {
      const item = next.find((row) => row.REQUIREMENT_ID === id);
      if (item) select(item);
    }
  };
  const select = (row: Requirement) => {
    ...
    setSelectedId(row.REQUIREMENT_ID); setDraft(value); setBaseline(JSON.stringify(value)); setSelectedFile(null);
  };
  useEffect(() => {
    let current = true;
    ...
    requirementApi.list(projectId).then((items) => { if (current) { setRows(items); setSelectedId(''); const value = emptyDraft(projectId); setDraft(value); setBaseline(JSON.stringify(value)); } })
      .catch((error: Error) => { if (current && projectId) setMessage({ type: 'error', text: error.message }); });
  ...
  const run = async (work: () => Promise<void>) => {
    setBusy(true);
    try { await work(); } catch (error) { setMessage({ type: 'error', text: error instanceof Error ? error.message : '작업을 완료하지 못했습니다.' }); }
  ...
  const save = (event: FormEvent<HTMLFormElement>) => {
    ...
    void run(async () => {
      const saved = selectedId ? await requirementApi.update(selectedId, draft) : await requirementApi.create(draft);
      await load(saved.REQUIREMENT_ID);
      setMessage({ type: 'success', text: '요구사항을 저장했습니다.' });
    });
  };
  const migrate = () => void run(async () => {
    ...
    for (const row of pendingLegacy) {
      ...
      if (saved.LEGACY_SOURCE_ID !== row.REQUIREMENT_ID) throw new Error(`${row.REQUIREMENT_ID} 이관 확인에 실패했습니다.`);
      count += 1;
    }
    await load();
    setMessage({ type: 'success', text: `${count}건 이관을 확인했습니다. 기존 브라우저 데이터는 보존됩니다.` });
  });
  ...
  return <div className="page standard-design-page standard-design-project-page sd-requirement-page">
    <PageHeader breadcrumbs={['Standard Design', '요구사항 관리']} rightContent={<div className="standard-design-header-actions"><ProjectContextSelector onSelected={() => confirmDiscard()} /><div className="standard-design-lifecycle-actions" aria-label="요구사항 기능">
      ...
      <button type="button" className="primary-button" data-action-code="CREATE" disabled={busy || !projectId} onClick={() => { if (confirmDiscard()) { setSelectedId(''); const value=emptyDraft(projectId); setDraft(value); setBaseline(JSON.stringify(value)); setSelectedFile(null); } }}>등록</button>
      ...
      <button type="button" className="danger-button" data-action-code="DELETE" disabled={busy || !selectedId} onClick={() => { if (selectedId && window.confirm('선택한 요구사항과 첨부파일을 삭제하시겠습니까?')) void run(async () => { await requirementApi.delete(selectedId); setSelectedId(''); const value=emptyDraft(projectId); setDraft(value); setBaseline(JSON.stringify(value)); await load(); setMessage({ type: 'success', text: '요구사항을 삭제했습니다.' }); }); }}>삭제</button>
    </div></div>} />
    {!projectId ? <p className="sd-project-empty-help">프로젝트 Context를 선택하세요.</p> : <>
      ...
      <ProjectListDetailWorkspace subject="요구사항" mode={mode} onModeChange={setMode} list={<div className="project-list-detail-workspace__list-content"><div className="sd-requirement-search"><input aria-label="요구사항 검색" placeholder="ID/요구사항명" value={keyword} onChange={(e) => setKeyword(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') setAppliedKeyword(keyword); }} /><button type="button" className="secondary-button" onClick={() => setAppliedKeyword(keyword)}>검색</button></div><DataTable title={`요구사항 목록 (${visibleRows.length}건)`} columns={columns} rows={visibleRows} getRowKey={(row) => row.REQUIREMENT_ID} onRowClick={(row) => { if (confirmDiscard()) select(row); }} getRowClassName={(row) => row.REQUIREMENT_ID === selectedId ? 'selected-row' : ''} emptyMessage="조회된 요구사항이 없습니다." /></div>}
        ...
        </div></form><section className="sd-requirement-attachments"><div className="sd-requirement-section-heading"><h2>첨부자료 ({attachments.length}건)</h2>{selectedId ? <label className="secondary-button">파일 추가<input type="file" multiple hidden disabled={busy} onChange={(e) => { const files=Array.from(e.target.files ?? []); e.target.value=''; void run(async () => { for (const file of files) await requirementApi.upload(selectedId,file); await load(selectedId); setMessage({ type: 'success', text: `${files.length}개 파일을 업로드했습니다.` }); }); }} /></label> : <span>요구사항 저장 후 첨부할 수 있습니다.</span>}</div><div className="sd-requirement-file-layout"><div className="sd-requirement-file-list">{attachments.map((file) => <div key={file.ATTACHMENT_ID} className={selectedFile?.ATTACHMENT_ID === file.ATTACHMENT_ID ? 'selected' : ''}><button type="button" onClick={() => setSelectedFile(file)}>{file.ORIGINAL_FILE_NAME} · {Math.round(file.FILE_SIZE / 1024)} KB</button><button type="button" aria-label={`${file.ORIGINAL_FILE_NAME} 삭제`} disabled={busy} onClick={() => { if (window.confirm('첨부파일을 삭제하시겠습니까?')) void run(async () => { await requirementApi.deleteFile(selectedId,file.ATTACHMENT_ID); setSelectedFile(null); await load(selectedId); }); }}>삭제</button></div>)}</div><div className="sd-requirement-preview">{selectedFile && selectedId ? ['image/png','image/jpeg','image/webp'].includes(selectedFile.MIME_TYPE) ? <img src={requirementApi.fileUrl(selectedId,selectedFile.ATTACHMENT_ID)} alt={selectedFile.ORIGINAL_FILE_NAME} /> : <a href={requirementApi.fileUrl(selectedId,selectedFile.ATTACHMENT_ID)}>파일 다운로드</a> : <span>파일을 선택하면 미리보기가 표시됩니다.</span>}</div></div></section><section className="sd-requirement-ai-slot"><h2>AI 요구사항 분석</h2><p>향후 분석 결과를 이 영역에서 검토합니다.</p></section></div>}
      />
    </>}
