...
const groupColumns: DataTableColumn<CodeGroup>[] = [
  { key: 'CODE_GROUP_ID', header: '그룹 ID', width: 140, render: row => row.CODE_GROUP_ID },
...
const codeColumns: DataTableColumn<Code>[] = [
  { key: 'CODE_ID', header: '코드 ID', width: 130, render: row => row.CODE_ID },
  { key: 'CODE', header: '코드', width: 100, render: row => row.CODE },
...
const attributeColumns: DataTableColumn<CodeAttributeDefinition>[] = [
  { key: 'ATTRIBUTE_CODE', header: '속성코드', width: 120, render: row => row.ATTRIBUTE_CODE },
...
export default function CodeManagePage() {
  ...
  const loadGroups = useCallback(async (search: Condition, preferred = '') => {
    if ((groups.dirty || attributes.dirty || codes.dirty) && !window.confirm('미저장 변경사항이 사라집니다. 계속하시겠습니까?')) return;
    try {
  ...
  const saveGroups = async () => {
    ...
    try {
      ...
      await loadGroups(condition, selectedGroupId);
  ...
  const toolbar = (add: () => void, remove: () => void, save: () => void, selected: Set<string>, dirty: boolean) => [
  ...
  return <section className="page code-manage-page multi-grid-page">
    ...
    <MasterDetailMultiGrid
      ...
      master={<BaseKitDataGrid programKey={PROGRAM_KEY} roleCode={ROLE} title="코드그룹 목록" columns={groupColumns} rows={groups.rows} getRowKey={row => row.__GRID_ROW_ID} currentRowKey={groups.rows.find(row => row.CODE_GROUP_ID === selectedGroupId)?.__GRID_ROW_ID} selectedRowKeys={groupSelected} onSelectedRowKeysChange={setGroupSelected} onRowClick={selectGroup} getRowClassName={rowClass(groups.getState)} editing={{ keys: groupColumns.map(column => column.key), onChange: (row, key, value) => groups.update(row.__GRID_ROW_ID, current => ({ ...current, [key]: value })), isEditable: (row, key) => groups.getState(row) !== 'DELETED' && (key !== 'CODE_GROUP_ID' || groups.getState(row) === 'INSERTED') }} toolbarActions={toolbar(() => groups.add({ ...managed, CODE_GROUP_ID: '', CODE_GROUP_NAME: '', DESCRIPTION: '' }), () => { groups.remove(groupSelected); setGroupSelected(new Set()); }, () => void saveGroups(), groupSelected, groups.dirty)} />}
      detailTop={<BaseKitDataGrid programKey={PROGRAM_KEY} roleCode={ROLE} title={`${selectedGroupId} 속성정의 목록`} columns={attributeColumns} rows={attributes.rows} getRowKey={row => row.__GRID_ROW_ID} selectedRowKeys={attributeSelected} onSelectedRowKeysChange={setAttributeSelected} getRowClassName={rowClass(attributes.getState)} editing={{ keys: attributeColumns.map(column => column.key), onChange: (row, key, value) => attributes.update(row.__GRID_ROW_ID, current => ({ ...current, [key]: key === 'SORT_ORDER' ? Number(value) : value })), isEditable: row => attributes.getState(row) !== 'DELETED' }} toolbarActions={toolbar(() => selectedGroupId ? attributes.add({ ...managed, ATTRIBUTE_DEF_ID: '', CODE_GROUP_ID: selectedGroupId, ATTRIBUTE_CODE: '', ATTRIBUTE_NAME: '', DATA_TYPE: 'STRING', CONTROL_TYPE: 'TEXT', DISPLAY_TYPE: 'TEXT', REQUIRED_YN: 'N', DEFAULT_VALUE: null, OPTION_SOURCE: null, SORT_ORDER: 0 }) : notify('코드그룹을 선택해 주세요.'), () => { attributes.remove(attributeSelected); setAttributeSelected(new Set()); }, () => void saveAttributes(), attributeSelected, attributes.dirty)} />}
      detailBottom={<BaseKitDataGrid programKey={PROGRAM_KEY} roleCode={ROLE} title={`${selectedGroupId} 공통코드 목록`} columns={codeColumns} fields={fields} rows={codes.rows} getRowKey={row => row.__GRID_ROW_ID} getFieldValue={(row, field) => row.ATTRIBUTE_VALUES?.[field.key]} selectedRowKeys={codeSelected} onSelectedRowKeysChange={setCodeSelected} getRowClassName={rowClass(codes.getState)} editing={{ keys: [...codeColumns.map(column => column.key), ...fields.map(field => `ATTRIBUTE_${field.key}`)], onChange: (row, key, value) => codes.update(row.__GRID_ROW_ID, current => key.startsWith('ATTRIBUTE_') ? { ...current, ATTRIBUTE_VALUES: { ...current.ATTRIBUTE_VALUES, [key.slice(10)]: value } } : { ...current, [key]: key === 'SORT_ORDER' ? Number(value) : value }), isEditable: (row, key) => codes.getState(row) !== 'DELETED' && (key !== 'CODE_ID' || codes.getState(row) === 'INSERTED') }} toolbarActions={toolbar(() => selectedGroupId ? codes.add({ ...managed, CODE_ID: '', CODE_GROUP_ID: selectedGroupId, CODE: '', CODE_NAME: '', SORT_ORDER: 0, ATTRIBUTE_VALUES: Object.fromEntries(fields.map(field => [field.key, field.defaultValue ?? ''])) }) : notify('코드그룹을 선택해 주세요.'), () => { codes.remove(codeSelected); setCodeSelected(new Set()); }, () => void saveCodes(), codeSelected, codes.dirty)} />}
    />
  </section>;
}
