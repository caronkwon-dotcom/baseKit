...
const ROLE = 'ADMIN';
const initialSearchCondition: Condition = { groupKeyword: '', codeName: '', useYn: '' };
const newRowDefaults = { USE_YN: 'Y' as UseYn, DEL_YN: 'N' as const, REG_DT: '', REG_BY: '', MOD_DT: '', MOD_BY: '' };
...
const toGroupSavePayload = (row: CodeGroup) => ({
  CODE_GROUP_ID: trimText(row.CODE_GROUP_ID), CODE_GROUP_NAME: trimText(row.CODE_GROUP_NAME),
  DESCRIPTION: trimText(row.DESCRIPTION), USE_YN: row.USE_YN,
});
const toAttributeSavePayload = (row: CodeAttributeDefinition) => ({
  ATTRIBUTE_CODE: trimText(row.ATTRIBUTE_CODE).toUpperCase(), ATTRIBUTE_NAME: trimText(row.ATTRIBUTE_NAME),
  DATA_TYPE: row.DATA_TYPE, CONTROL_TYPE: row.CONTROL_TYPE, DISPLAY_TYPE: row.DISPLAY_TYPE,
  REQUIRED_YN: row.REQUIRED_YN, DEFAULT_VALUE: row.DEFAULT_VALUE, OPTION_SOURCE: row.OPTION_SOURCE,
  SORT_ORDER: Number(row.SORT_ORDER), USE_YN: row.USE_YN,
});
const toCodeSavePayload = (row: Code, groupId: string) => ({
  CODE_ID: trimText(row.CODE_ID), CODE_GROUP_ID: groupId, CODE: trimText(row.CODE),
  CODE_NAME: trimText(row.CODE_NAME), SORT_ORDER: Number(row.SORT_ORDER), USE_YN: row.USE_YN,
...
export default function CodeManagePage() {
  const [condition, setCondition] = useState(initialSearchCondition);
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      if (!initialLoad.current) { initialLoad.current = true; void loadGroups(initialSearchCondition); }
    }, 0);
  ...
  const saveGroups = async () => {
    ...
    setSaving(true);
    try {
      await coreCodeApi.saveGroupBatch({ INSERTED: groups.changeSet.INSERTED.map(toGroupSavePayload), UPDATED: groups.changeSet.UPDATED.map(toGroupSavePayload), DELETED: groups.changeSet.DELETED });
  ...
  const saveAttributes = async () => {
    ...
    setSaving(true);
    try {
      await coreCodeApi.saveAttributeBatch(selectedGroupId, { INSERTED: attributes.changeSet.INSERTED.map(toAttributeSavePayload), UPDATED: attributes.changeSet.UPDATED.map(row => ({ ATTRIBUTE_DEF_ID: row.ATTRIBUTE_DEF_ID, VALUE: toAttributeSavePayload(row) })), DELETED: attributes.changeSet.DELETED });
  ...
  const saveCodes = async () => {
    ...
    setSaving(true);
    try {
      const toPayload = (row: Code) => toCodeSavePayload(row, selectedGroupId);
      await coreCodeApi.saveCodeBatch(selectedGroupId, { INSERTED: codes.changeSet.INSERTED.map(toPayload), UPDATED: codes.changeSet.UPDATED.map(toPayload), DELETED: codes.changeSet.DELETED });
  ...
  const removeGroups = () => { groups.remove(groupSelected); setGroupSelected(new Set()); };
  const revertGroups = () => { groups.revert(groupSelected); setGroupSelected(new Set()); };
  const removeAttributes = () => { attributes.remove(attributeSelected); setAttributeSelected(new Set()); };
  const revertAttributes = () => { attributes.revert(attributeSelected); setAttributeSelected(new Set()); };
  const removeCodes = () => { codes.remove(codeSelected); setCodeSelected(new Set()); };
  const revertCodes = () => { codes.revert(codeSelected); setCodeSelected(new Set()); };

  const addAttribute = () => selectedGroupId ? attributes.add({
    ...newRowDefaults, ATTRIBUTE_DEF_ID: '', CODE_GROUP_ID: selectedGroupId,
    ATTRIBUTE_CODE: '', ATTRIBUTE_NAME: '', DATA_TYPE: 'STRING', CONTROL_TYPE: 'TEXT',
    DISPLAY_TYPE: 'TEXT', REQUIRED_YN: 'N', DEFAULT_VALUE: null, OPTION_SOURCE: null, SORT_ORDER: 0,
  }) : notify('코드그룹을 선택해 주세요.');
  const addCode = () => selectedGroupId ? codes.add({
    ...newRowDefaults, CODE_ID: '', CODE_GROUP_ID: selectedGroupId, CODE: '', CODE_NAME: '',
    SORT_ORDER: 0, ATTRIBUTE_VALUES: Object.fromEntries(fields.map(field => [field.key, field.defaultValue ?? ''])),
  }) : notify('코드그룹을 선택해 주세요.');
  const updateCodeCell = (row: TrackedGridRow<Code>, key: string, value: string) => {
    const metadataKey = fromMetadataColumnKey(key);
    codes.update(row.__GRID_ROW_ID, current => metadataKey === null
      ? applyGridValue(current, codeColumns, key, value)
      : { ...current, ATTRIBUTE_VALUES: { ...current.ATTRIBUTE_VALUES, [metadataKey]: value } });
  };
  ...
  return <section className="page code-manage-page multi-grid-page">
    ...
    <SearchPanel
      rows={1} fields={searchFields} value={condition} initialValue={initialSearchCondition}
      onValueChange={setCondition} onSearch={value => void loadGroups(value, selectedGroupId)}
      onReset={value => void loadGroups(value)}
    />
    <MasterDetailMultiGrid
      message={message ? <BaseKitMessage type={message.type} message={message.text} /> : null}
      master={<BaseKitDataGrid
        programKey={PROGRAM_KEY} roleCode={ROLE} title="코드그룹 목록"
        columns={groupColumns} rows={groups.rows}
        ...
        toolbarActions={toolbar(
          ...
          removeGroups, revertGroups, () => void saveGroups(), groupSelected, groups.hasChanges(groupSelected), groups.dirty,
        )}
      />}
      detailTop={<BaseKitDataGrid
        programKey={PROGRAM_KEY} roleCode={ROLE} title={`${selectedGroupId} 속성정의 목록`}
        ...
        toolbarActions={toolbar(
          ...
          attributeSelected, attributes.hasChanges(attributeSelected), attributes.dirty,
        )}
      />}
      detailBottom={<BaseKitDataGrid
        programKey={PROGRAM_KEY} roleCode={ROLE} title={`${selectedGroupId} 공통코드 목록`}
        columns={codeColumns} fields={fields} rows={codes.rows}
        ...
        toolbarActions={toolbar(
          ...
          codeSelected, codes.hasChanges(codeSelected), codes.dirty,
        )}
      />}
    />
  </section>;
