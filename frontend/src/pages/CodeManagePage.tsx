...
export default function CodeManagePage() {
  ...
  return <section className="page code-manage-page multi-grid-page">
    ...
    <MasterDetailMultiGrid
      ...
      detailTop={<BaseKitDataGrid programKey={PROGRAM_KEY} roleCode={ROLE} title={`${selectedGroupId} 속성정의 목록`} columns={attributeColumns} rows={attributes.rows} getRowKey={row => row.__GRID_ROW_ID} selectedRowKeys={attributeSelected} onSelectedRowKeysChange={setAttributeSelected} getRowClassName={rowClass(attributes.getState)} editing={{ keys: attributeColumns.map(column => column.key), onChange: (row, key, value) => attributes.update(row.__GRID_ROW_ID, current => ({ ...current, [key]: key === 'SORT_ORDER' ? Number(value) : value })), isEditable: row => attributes.getState(row) !== 'DELETED' }} toolbarActions={toolbar(() => selectedGroupId ? attributes.add({ ...managed, ATTRIBUTE_DEF_ID: '', CODE_GROUP_ID: selectedGroupId, ATTRIBUTE_CODE: '', ATTRIBUTE_NAME: '', DATA_TYPE: 'STRING', CONTROL_TYPE: 'TEXT', DISPLAY_TYPE: 'TEXT', REQUIRED_YN: 'N', DEFAULT_VALUE: null, OPTION_SOURCE: null, SORT_ORDER: 0 }) : notify('코드그룹을 선택해 주세요.'), () => { attributes.remove(attributeSelected); setAttributeSelected(new Set()); }, () => { attributes.revert(attributeSelected); setAttributeSelected(new Set()); }, () => void saveAttributes(), attributeSelected, attributes.hasChanges(attributeSelected), attributes.dirty)} />}
