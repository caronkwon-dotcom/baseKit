...
| 논리명 | 물리명 | 도메인 | 상태 |
|---|---|---|---|
| 등록일시 | `CREATED_AT` | AT | 기존 용어 |
| 등록자 | `CREATED_BY` | BY | 기존 용어 |
| 수정일시 | `UPDATED_AT` | AT | 기존 용어 |
| 수정자 | `UPDATED_BY` | BY | 기존 용어 |

삭제는 물리 삭제보다 `사용여부`와 상태 변경을 우선한다. 보안·이력 테이블의 감사 필드는 별도 정책 확정 시 확장한다.
