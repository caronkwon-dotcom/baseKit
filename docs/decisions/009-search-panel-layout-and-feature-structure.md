...
- `SearchPanel.rows`는 `1 | 2 | 3 | 4 | 5`만 허용한다.
- 조건 상한은 단수당 4개이며 최대 5단 20개다.
- 상한을 넘으면 공통 컴포넌트가 오류를 발생시킨다.
- 작은 화면에서는 조건과 Action을 한 열로 전환한다.
- 검색 라벨과 입력 Control은 각 조건 Cell 안에서 가로 한 줄로 배치한다.
- 개발자가이드 샘플은 1~5단 미리보기 전환을 제공하되 실제 업무 Page는 단수를 config에 고정한다.
- 1단은 접기 기능 없이 조회 버튼과 초기화 아이콘을 같은 줄에 표시한다.
- 2단은 조회 버튼을 위에 두고 초기화·접기 아이콘을 아래 50%씩 배치한다.
- 3단 이상은 조회 버튼을 검색조건 2단 높이로 강조하고 그 아래 초기화·접기 아이콘을 배치한다.
- 2단 이상을 접으면 첫 1단만 유지하고 숨겨진 조건은 `+N 추가조건`으로 표시한다.

### 개발자 계약

개별 Page는 검색 Control과 공통 버튼 JSX를 작성하지 않는다. 다음 설정만 제공한다.

```text
fields: key, label, controlType, placeholder, options
value / initialValue
onValueChange / onSearch / onReset
rows
```

입력 Control, 조회·초기화, 접기·펼치기와 조건 수 Guard는 `SearchPanel`이 생성한다.
...
- 4~5단 업무화면도 동일 Layout과 접기 정책을 사용한다.
