...
BaseKit은 DB 도입 전이며 `frontend/meta/terms.json`, `frontend/meta/domains.json`에 최소 샘플만 관리한다. 행정안전부 공공표준용어 8차 데이터는 13,176건으로, 원본 전체를 즉시 BaseKit 표준으로 적용하기에는 기존 명명 규칙과 충돌하고 폐기·변경 이력도 포함한다.

## 결정

- 공공표준 CSV는 수정하지 않는 참조 원본으로 보존한다.
- 정제 결과는 원본 전체 복제가 아니라 검토한 항목의 상태와 BaseKit 적용값만 `frontend/meta/term-curation.json`에 저장한다.
...
frontend/meta/term-curation.json                검토 진행 및 정제 결과
frontend/meta/terms.json                        추후 승인된 BaseKit 표준 용어
frontend/meta/domains.json                      추후 승인된 BaseKit 표준 도메인
```
