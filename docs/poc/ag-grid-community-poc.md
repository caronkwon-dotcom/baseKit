...
`CODE_ATTRIBUTE_DEF → 기존 toFieldDefinitions → FieldDefinition → MetadataAgGrid → gridColumnAdapter → AG Grid Community`

- `BaseKitDataGrid`: 기존 ProgramDataGrid의 제목·건수·권한·CRUD Toolbar를 재사용하고 내부 GridTable에서 AG Grid API와 선택 동기화를 처리한다.
- `gridColumnAdapter`: 고정 DataTableColumn과 동적 FieldDefinition을 AG Grid ColDef로 변환. 숫자/불리언 원값과 Badge/Color 표현을 분리한다. 안정된 colId와 initialWidth/initialFlex로 재렌더링 시 사용자가 조정한 컬럼 상태를 유지한다.
- `ProgramDataGrid`에는 제품 독립적인 선택적 `renderTable` 연결점만 추가했다. 기본값은 기존 DataTable이다. DataTableProps는 타입 export만 추가했다.
- 기존 MetadataDataGrid는 변경·삭제하지 않았다. PROGRAM, MENU, Standard Design은 기존 Grid를 계속 사용한다.
...
- `frontend/src/components/grid/MetadataAgGrid.tsx`
